        ...
        {tab !== 'home' && (
          <div className="md:hidden border-t border-gray-50 flex items-center gap-2 py-2">
            ...
            <span className="text-sm font-semibold text-blue-600">
              {tab === 'new' ? '+ Nouveau colis' : tab === 'caisse' ? (profile?.role === 'chef_agence' ? `Caisse Agence ${profile?.city || ''}` : 'Ma Caisse') : tab === 'cod' ? 'RETOUR FOND Clients' : tab === 'clientportdu' ? '🏦 Ports en compte clients' : tab === 'portpayecheque' ? '📋 Ports payés chèque' : tab === 'invoices' ? '🧾 Factures' : tab === 'clients' ? 'Mes clients' : tab === 'charge' ? 'Feuille de charge' : tab === 'secteurs' ? 'Secteurs' : tab === 'drivers' ? 'Port dû' : tab === 'arrivage' ? 'Arrivages' : tab === 'retours' ? '↩️ Retours' : tab === 'lostparcels' ? '🚨 Colis perdus' : tab === 'modifications' ? 'Modif. clients' : tab === 'aideagents' ? 'Aide Agents' : tab === 'dashboard' ? 'Dashboard' : tab === 'notes' ? 'Notes agents' : 'Expéditions'}
            </span>
          </div>
        )}
          ...
          <div className="md:hidden border-t border-gray-100 py-2 space-y-1 max-h-[calc(100vh-120px)] overflow-y-auto">
            ...
            {(profile?.role === 'chef_agence' || profile?.role === 'agentpro') && (
              <>
                ...
                <button
                  onClick={() => { setTab('portpayecheque'); setMenuOpen(false) }}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition ${tab === 'portpayecheque' ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                  📋 Ports payés chèque
                </button>
